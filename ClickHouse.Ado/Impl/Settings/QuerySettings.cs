...
internal class QuerySettings {
    ...
    internal async Task Write(ProtocolFormatter formatter, CancellationToken cToken) {
        foreach (var settingValue in _settings) {
            await formatter.WriteString(settingValue.Key, cToken);
            await settingValue.Value.Write(formatter, cToken);
        }

        await formatter.WriteString("", cToken);
    }
}